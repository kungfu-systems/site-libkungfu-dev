...
| 2026-08-16 | open-minor | `dogfood-featured-observation/v1` | `/dogfood/`, Observation snapshot selector, root manifest observed-evidence projection | additive | The Dogfood reader pins the 2026-08-03 immutable observation as an explicitly dated featured default while weekly latest evidence and append-only history continue unchanged; selecting another window updates the headline context, metrics, provenance, and machine route together. | |
...
| 2026-08-15 | open-minor | `kfd-agent-hub-three-axis/v1` | KFD independent implementation path, `/agent-hub/`, KFD manifest, KFD llms | additive | The site projects Alpha.67 behavior, evidence, and authority as separate axes before commands, preserves distinct reference and fail-closed starter paths with exact exit expectations, and keeps verification validity separate from behavior conformance and certification. | |
