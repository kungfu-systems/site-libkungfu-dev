...
authority. The Core page consumes the exact pinned
...
owns only reader framing, routing, and rendering. Until registry publication is
required, the exact package artifact is vendored at
`vendor/kungfu-tech-site-4.0.0-alpha.1.tgz`. It is generated from one clean
Kungfu commit, locked by pnpm integrity, and verified against the package's
...
```text
kungfu framework/site -> exact source-bound package tarball -> site vendor pickup -> core.libkungfu.dev
